...
if(config.AUTO_REACT) await conn.sendMessage(from, { react: {  text: "🎧", key: doc.key } } ) 
...
await conn.sendMessage(from, { delete: docsongdown.key })
const docsongup = await conn.sendMessage(from , { text: config.SONG_UP }, { quoted: mek } )
const doc = await conn.sendMessage(from ,{ audio: { url: docsong.mp3 }, mimetype: 'audio/mp4' } , { quoted: mek })
if(config.AUTO_REACT) await conn.sendMessage(from, { react: {  text: "🎧", key: doc.key } })
await conn.sendMessage(from, { delete: docsongup.key })}
  if (config.WORKTYPE == 'public') {
     if(config.AUTO_REACT) await conn.sendMessage(from, { react: {  text: "🎧", key: mek.key } } ) 
...
const srcres = await conn.sendMessage(from , { text: '╔═══════════════╗\n║  *🎞️AQUABOT YT SEARCH🎞️* ║\n╚═══════════════╝\n\n' + mesaj }, { quoted: mek } )
  if(config.AUTO_REACT) await conn.sendMessage(from, { react: {  text: "⌨️", key: srcres.key } })
